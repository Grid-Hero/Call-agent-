...
class Orchestrator:
    ...
    # --- Ablauf -------------------------------------------------------------
    def handle_incoming(self, form: dict) -> str:
        ...
        return self.adapter.greeting_response(
            self.directory.greeting, self._url("/voice/handle"), self.settings.agent_language
        )
    ...
    async def _apply_decision(self, session: CallSession, decision: RoutingDecision) -> str:
        ...
        if decision.action == Action.CONTINUE:
            return self.adapter.continue_response(
                decision.reply_text, self._url("/voice/handle"), lang
            )
        ...
        if decision.action == Action.TRANSFER:
            ...
            if transfer_ok and phone:
                ...
                return self.adapter.transfer_response(
                    decision.reply_text, phone, lang, self._url("/voice/after-transfer")
                )
        ...
        if decision.action == Action.MESSAGE:
            await self._finish_with_message(session)
            return self.adapter.hangup_response(decision.reply_text, lang)
    ...
    async def handle_after_transfer(self, form: dict) -> str:
        ...
        return self.adapter.hangup_response("Vielen Dank für Ihren Anruf. Auf Wiederhören!", self.settings.agent_language)
